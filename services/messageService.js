    ...
    async extractMessageData(messageBody, date, sender) {
        try {
            ...
            if (amount === null || isNaN(amount)) {
                let match = messageBody.match(/amount[:\s]+([\d.]+)/i);
                if (match) {
            ...
            if (amount === null || isNaN(amount)) {
                let match = messageBody.match(/(?:EGP|LE|جنيه)\s*([\d,.]+)|([\d,.]+)\s*(?:EGP|LE|جنيه)/i); 
                    if (match) {
                        let rawAmount = match[1] || match[2];
                    ...
                    if (messageBody.includes("إضافة تحويل") || messageBody.includes("إيداع") || messageBody.includes("رد مبلغ") || messageBody.includes("إستلام")
                        || messageBody.includes("added") || messageBody.includes("deposited") || messageBody.includes("received") || messageBody.includes("credited") || messageBody.includes("charged")) {
                        type = "received"
                    } else if (messageBody.includes("سحب") || messageBody.includes("خصم") || messageBody.includes("تنفيذ تحويل") || messageBody.includes("transferred")
                        || messageBody.includes("withdrawal") || messageBody.includes("deducted") || messageBody.includes("transfer") || messageBody.includes("paid") || messageBody.includes("debited")) {
                        type = "sent"
                    }
